...
# `_COMPATIBLE_STABLE_DIFFUSION_SCHEDULERS`는 구버전 diffusers의 private 심볼이며
# 최신 버전(>=0.20 부근)에서는 제거되었다. 이 scheduler 는 policy 의 noise scheduler 로만
# 사용되므로 SD 파이프라인 호환 목록은 필요 없고, 비어있어도 안전하다.
try:
    from diffusers.utils import _COMPATIBLE_STABLE_DIFFUSION_SCHEDULERS  # type: ignore
except ImportError:
    _COMPATIBLE_STABLE_DIFFUSION_SCHEDULERS: list = []
...
# 수정 for v-prediction
def betas_for_alpha_bar(num_diffusion_timesteps, max_beta=0.999, prediction_type="epsilon") -> torch.Tensor:
    """
    ...
    def alpha_bar(time_step):
        ...
        # epsilon or sample prediction (improved DDPM)
        elif prediction_type == "epsilon" or prediction_type == "sample":
...
class DDIMPIGDMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if trained_betas is not None:
            ...
            self.betas = betas_for_alpha_bar(num_train_timesteps, prediction_type=self.config.prediction_type)
        else:
    ...
    def step(
        ...
        if self.config.prediction_type == "epsilon":
            ...
            # x_0 = cos(phi) * x_t - sin(phi) * v_t
            pred_original_sample = (alpha_prod_t**0.5) * sample - (beta_prod_t**0.5) * model_output
            # epsilon = sin(phi) * x_t + cos(phi) * v_t
        ...
        if self.config.clip_sample:   # True
