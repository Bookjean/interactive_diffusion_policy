...
from diffusion_policy.policy.diffusion_unet_hybrid_image_policy_pigdm import DiffusionUnetHybridImagePigdmPolicy
...
class TrainDiffusionUnetHybridPigdmWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # configure model
        self.model: DiffusionUnetHybridImagePigdmPolicy = hydra.utils.instantiate(cfg.policy)   # cfg의 policy: 아래 긁어옴

        # use_ema가 True이면 원본 모델 self.model을 copy함; ema는 최근 data에 지수적으로 큰 가중치로 평균을 매김, 추가적인 도구
        self.ema_model: DiffusionUnetHybridImagePigdmPolicy = None
