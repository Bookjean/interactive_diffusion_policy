...
from ddim_scheduler_pigdm import DDIMPIGDMScheduler
from diffusion_policy.real_world.real_inference_util import get_real_relative_obs_dict, get_abs_action_from_relative, get_relative_action_from_abs
...
class DiffusionUnetHybridImagePigdmPolicy(BaseImagePolicy):
    def __init__(self, 
        ...
        self.action_gripper_repr = action_gripper_repr

        # RotationTransformer 초기화 (rb10_eval_real_robot.py와 동일한 패턴)
        self.rot_quat2mat = RotationTransformer('quaternion', 'matrix')
        self.rot_6d2mat   = RotationTransformer('rotation_6d', 'matrix')
        self.rot_mat2target = {}
        ...
        if action_pose_repr == 'relative':
            action_rotation_rep = shape_meta['action'].get('rotation_rep', None)
            if action_rotation_rep is None:
                raise KeyError(
                    "shape_meta.action.rotation_rep is required when "
                    "action_pose_repr='relative'")
            self.rot_mat2target['action'] = RotationTransformer('matrix', action_rotation_rep)
    ...
    def get_abs_action(self, action: np.ndarray):
        self.prev_unnormalized_abs_action = action
